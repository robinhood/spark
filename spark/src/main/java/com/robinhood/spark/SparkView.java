...
public class SparkView extends View implements ScrubGestureDetector.ScrubListener {
	...
	private AnimaType animationType;
...
//		animateChanges = a.getBoolean(R.styleable.spark_SparkView_spark_animateChanges, false);
		animationType = AnimaType.values()[a.getInt(R.styleable.spark_SparkView_spark_animationType, 0)];
...
//			}
	...
	/**
	 * Type of animation to user when data changes
	 */
	public AnimaType getAnimationType() {
	 ...
	 */
	public void setAnimationType(AnimaType animationType) {
	 ...
	 * @deprecated User getAnimationType instead.
	 */
	 ...
	 * @deprecated User setAnimationType instead.
	 */
	@Deprecated
	...
	private void doPathAnimation() {
		...
		if(animationType == AnimaType.LINE) {
			pathAnimator = getAnimationLine();
		}
		else if(animationType == AnimaType.POINT) {
			pathAnimator = getAnimationPoint();
		}
	...
	private ValueAnimator getAnimationLine() {
		...
		ValueAnimator lineAnimator = ValueAnimator.ofFloat(0, endLength);
	...
	private ValueAnimator getAnimationPoint() {

		ValueAnimator pointAnimator = ValueAnimator.ofFloat(0, 1);
		...
		pointAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				for(int count = 0; count < size; count++) {
					...
					step = currYPoints.get(count) - oldY;
					y = (step * animatedValue) + oldY;

					if (count == 0) {
						renderPath.moveTo(xPoints.get(count), y);
					}
					else {
						renderPath.lineTo(xPoints.get(count), y);
					}
	...
	private final DataSetObserver dataSetObserver = new DataSetObserver() {
		...
		public void onChanged() {
			...
			if (animationType != AnimaType.NONE) {
				doPathAnimation();
			}
		}
